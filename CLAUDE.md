...
`n` = previous job, `p` = next job (vim-reversed from numeric intuition). `Tab` cycles focus across blocks (JobList → Details → Stdout → Stderr → JobList). The right-hand panel is a tab container — Details and Output (stdout+stderr) are separate tabs; the active tab follows focus, and clicking the tab title switches it. `d` removes the current job from view. `Enter` opens the focused log in the editor. Arrow keys act on the focused block — navigating jobs in the list, or scrolling in a log panel (which toggles per-panel scroll mode); scrolling back to the bottom or pressing `q` exits scroll mode. Mouse click sets focus (clicking a row in the JobList also selects that job); mouse scroll scrolls the panel under the cursor. Layout is fixed (JobList on top, info panel below). `+`/`=` zoom the focused panel to fullscreen; `-`/`_` exit zoom. `Ctrl+C` always quits.
